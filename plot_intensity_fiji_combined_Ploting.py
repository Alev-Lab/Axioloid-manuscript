...
#colorlist = ["fuchsia","darkturquoise"]
colorlist = ["red","g"]
...
def peak_detection(signal):
    indexes = dp.detect_peaks(signal, mph=0.5, mpd=10)
    return indexes
...
def fig_plot(df, x_label,y_label, plot_y, ave_op, norm_op, agg_op, italic_op, ci,xmin,xmax, ymin, ymax):
    ...
    #plt.ylabel('Intensity (a.u.)',fontsize=18)
    plt.xlabel('Distance from posterior end (\N{greek small letter mu}m)', fontsize=18)
...
def do_plot(df_tmp ,base, x_label,y_label, sma_window, trend_window,
    ...
    #df_tmp = df_tmp.rename(columns={'Time (h)':'Time (individual_well) (h)'})
    #df_tmp.insert(0, 'Date', name[0:9])
    #df_tmp.insert(0, 'Sample name', name[0:name.find('_ch#')])
    ...
    f = interpolate.interp1d(x, data, kind='cubic',fill_value='extrapolate')
    x_norm = x_ref[x_ref<=max(x)]
    data = f(x_ref)[x_ref<=max(x)]
...
if __name__ == '__main__':
    ...
    #argv = ["","/Volumes/Hamidi.S_Alev_lab/PROJECT_0_AXIOLOIDS/Confocal/LSM980/intensity_measurments_compiled/409b2+MG+RA_120h/test/"]
    ...
    if args.input:
        ...
        file_ref=glob.glob(folder+'/*BRA.csv')
